...
const SketchedCircle = ({ delay = 0 }) => (
  <svg className="absolute inset-0 w-full h-full pointer-events-none -z-10 text-primary/30" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="2">
    <motion.path
      ...
      style={{ strokeDasharray: "5 5" }} // Dashed for sketch look
    />
    <motion.path
...
const SketchedArrow = ({ className = "" }) => (
  <svg className={`overflow-visible ${className}`} width="60" height="20" viewBox="0 0 60 20" fill="none" stroke="currentColor" strokeWidth="2">
    <motion.path
      d="M5,10 Q30,5 55,10"
...
const Approach = () => {
  return (
    <section id="approach" className="py-24 lg:py-32 bg-foreground text-primary-foreground overflow-hidden">
      <div className="container mx-auto px-6 lg:px-12">
        ...
        >
          {/* Decorative doodle */}
          <motion.svg
            className="absolute -left-12 -top-12 w-24 h-24 text-primary/40 rotate-12"
            viewBox="0 0 100 100"
            initial={{ opacity: 0 }}
          ...
          <span className="text-sm font-semibold text-primary uppercase tracking-wider mb-4 block flex items-center gap-2">
            Our Approach
            <motion.div
              className="h-px bg-primary w-12"
              initial={{ width: 0 }}
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-6 font-display">
            A human process for <span className="relative inline-block">
              ...
              <motion.svg className="absolute w-full h-3 -bottom-1 left-0 text-primary" viewBox="0 0 100 10" preserveAspectRatio="none">
                <motion.path d="M0,5 Q50,10 100,5" fill="none" stroke="currentColor" strokeWidth="3" initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} transition={{ duration: 0.8, delay: 0.5 }} />
              </motion.svg>
            </span> results.
          </h2>
          ...
          {steps.map((step, index) => (
            ...
            >
              <div className="relative w-24 h-24 mb-6 flex items-center justify-center">
                ...
                <span className="text-4xl font-bold text-primary-foreground font-display z-10 group-hover:scale-110 transition-transform duration-300">
                  {step.number}
                </span>
              </div>
              ...
              <p className="text-sm text-primary-foreground/60 leading-relaxed max-w-[200px] mx-auto opacity-80 group-hover:opacity-100 transition-opacity">
                {step.description}
              </p>

              {/* Mobile Arrows */}
              {index < steps.length - 1 && (
                <div className="lg:hidden my-4 text-primary/30 rotate-90">
                  <SketchedArrow />
                </div>
              )}
            </motion.div>
